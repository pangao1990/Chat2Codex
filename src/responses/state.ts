...
function schedulePersist(): void {
  ...
  // Resolve the target path now: tests may swap CHAT2CODEX_HOME before the
...
export function flushResponseState(): void {
  if (!persistTimer) return;
  // Use the path captured when the write was scheduled; CHAT2CODEX_HOME may have moved.
